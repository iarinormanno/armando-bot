...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0]) {
      ...
      user = message.mentions.members.first() || await message.guild.members.fetch(args[0]).catch(err => { return message.channel.send(":x: Impossibile trovare questo utente") })
    }

    if (!user) {
      return message.channel.send(":x: Impossibile trovare questo utente!")
    }
